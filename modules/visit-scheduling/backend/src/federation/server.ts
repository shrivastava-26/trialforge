/**
 * Federation-compatible server for visit-scheduling subgraph.
 * Runs on PORT_FEDERATION (default 4141).
 */
...
async function start() {
  const dbPath = process.env.DB_PATH ?? path.join(__dirname, '..', '..', 'data', 'visit-scheduling.db');
  ...
  const port = Number(process.env.PORT_FEDERATION ?? 4141);
  app.listen(port, () => {
    console.log(`[visit-scheduling] Federation subgraph at http://localhost:${port}/graphql`);
  });
}

start().catch((err) => {
  console.error('[visit-scheduling] Federation server failed to start', err);
  process.exit(1);
});
